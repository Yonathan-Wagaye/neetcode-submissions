...
class MyStack:
    ...
    def push(self, x: int) -> None:
        ...
        print(f"After pushing {x}: Queue 1: {self.queueOne.array}, Queue 2: {self.queueTwo.array}")
        
    def pop(self) -> int:
        if self.queueTwo.isEmpty():
            for _ in range(self.queueOne.size - 1):
                self.queueTwo.pushToBack(self.queueOne.popFromFront())
            print(f"After poping: Queue 1: {self.queueOne.array}, Queue 2: {self.queueTwo.array}")
    ...
    def top(self) -> int:
        if self.queueTwo.isEmpty():
            ...
            print(f"After stack.top(): Queue 1: {self.queueOne.array}, Queue 2: {self.queueTwo.array}, lastElem: {lastElem}")
            return lastElem
        else:
            for _ in range(self.queueTwo.size - 1):
                self.queueOne.pushToBack(self.queueTwo.popFromFront())
